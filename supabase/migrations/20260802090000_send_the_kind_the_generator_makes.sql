...
-- Look for the content kind the product actually produces.
--
-- `buildContentPack` emits five kinds — `post`, `short_post`, `story`,
-- `video_script` and `direct_message` — and only the last of those is a message
-- to a person. The expansion looked for `message`, a kind nothing in the
-- application generates and the Content Studio has no label for; it exists only
-- in `supabase/seed.sql`. So a campaign whose copy the owner had written and
-- approved through the real screen still refused to send, reporting
-- `no_approved_content` while the approved content sat right there.
--
-- The legacy `message` stays accepted so the seeded campaign keeps working
-- until the seed is regenerated.
