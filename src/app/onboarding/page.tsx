...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ step?: string; saved?: string }> }) {
  ...
  return <main id="main-content" className="min-h-[100dvh] bg-background px-4 py-8 sm:py-12">
    ...
    <form action={saveOnboardingStep} className="mt-6 rounded-3xl border border-border bg-surface p-6 shadow-xl sm:p-8">
      ...
      {step === 6 && <fieldset className="grid gap-5"><legend className="text-xl font-bold">Каналы и стартовые данные</legend><div><p className="mb-2 text-sm font-semibold">Каналы</p><div className="grid grid-cols-2 gap-2">{['telegram','email','instagram'].map((channel) => <label key={channel} className="flex min-h-11 items-center gap-2 rounded-xl border border-border px-3"><input type="checkbox" name="channels" value={channel} defaultChecked={(draft.channels ?? ['telegram']).includes(channel)} />{channel}</label>)}</div></div><label className="grid gap-2 text-sm font-semibold">Тон бренда<input name="brandVoice" defaultValue={String(draft.brandVoice ?? 'дружелюбный и конкретный')} className={field} /></label><div><p className="mb-1 text-sm font-semibold">С каких данных начнём?</p><p className="mb-3 text-xs leading-5 text-muted-foreground">По умолчанию заведение создаётся пустым: своя база клиентов, свои продажи, своя аналитика. Ничего чужого в нём не появится.</p>{([['manual','Начать с чистого листа','Пустая база клиентов, пустые продажи, пустая аналитика. Всё наполнится вашими данными.'],['csv','Начать с чистого листа и импортировать CSV','То же самое, плюс мастер импорта клиентов и продаж сразу после настройки.'],['demo','Заполнить демонстрационными данными','Скопирует витрину TAMYR Coffee — чужих гостей, чеки и отчёты. Только чтобы посмотреть продукт: заведение будет помечено DEMO, и это не ваши цифры.']] as const).map(([code,label,hint]) => <label key={code} className="mb-2 grid min-h-12 grid-cols-[auto_1fr] items-start gap-3 rounded-xl border border-border p-4"><input type="radio" name="importMode" value={code} defaultChecked={(draft.importMode ?? 'manual') === code} required className="mt-1" /><span><span className="block text-sm font-semibold">{label}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{hint}</span></span></label>)}</div><input type="hidden" name="dataSources" value="onboarding" /></fieldset>}
