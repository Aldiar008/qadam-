...
> **Статусы ниже — снимок на 30 июля, а не текущее состояние.** С тех пор поставлены
> Telegram Mini App, живые рекомендации из сигналов, дашборды аналитики, автообновление
> материалов, парсинг закупок, подбор инструментов по профилю и история действий; приёмка
> выросла до пяти наборов против развёрнутого стенда. Актуальная сверка с положением
> турнира — **`TOURNAMENT_COMPLIANCE.md`**. Этот файл сохранён как след аудита: строка
> отсюда меняется только вместе с проверкой, которая доказывает изменение, и переписывать
> её задним числом было бы подделкой истории.
