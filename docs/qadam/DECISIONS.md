...
## ADR-020 — Telegram Mini App как первая поверхность для гостя

- Date: 2026-08-03
- Status: accepted
- Decision: гость видит свою карту внутри Telegram по адресу `/tg`, а не на
  отдельном сайте. Подпись `initData` проверяется по схеме Telegram и один раз
  обменивается на короткую подписанную куку, ограниченную путём `/tg`. CSP
  разрешает встраивание ровно для этого префикса и ровно с доменов Telegram;
  `script-src` не ослабляется — страница читает фрагмент URL сама, без стороннего
  SDK.
- Reason: гостевой сессии в продукте не было вообще: клиент опознавался по хешу
  контакта на каждый запрос, и это правильно для одноразового действия по QR, но
  не для экрана, по которому человек ходит. Встраивание — не уступка: Telegram в
  вебе рендерит Mini App в iframe, и `frame-ancestors 'none'` заблокировал бы его
  молча, оставив приложение работающим на телефоне и сломанным в браузере.
- Consequence: ни бизнес, ни клиент никогда не берутся из запроса — только из
  связки чата, поэтому назвать чужой идентификатор бесполезно, и набор это
  проверяет. Погашение награды из приложения проходит те же проверки баланса,
  остатка и идемпотентности, что и по QR. Владелец подтверждает запуск тем же
  путём, что в кабинете и в чате: `launch_contract_from_chat` заново проверяет
  роль, Margin Shield и правило «один запуск на контракт».

## ADR-021 — Автоматизация обязана оставить после себя действие

- Date: 2026-08-03
- Status: accepted
- Decision: правило, объявившее действием «предложить кампанию», создаёт
  рекомендацию с посчитанной экономикой. Правило, ничего не нашедшее, молчит.
  Ни один режим не ставит сообщение в очередь без подтверждения владельца, и ни
  одно правило больше не сообщает об отправке, которой не было.
- Reason: девять шаблонов из десяти считали строки и писали уведомление —
  «вернуть спящих» находило 64 человека и выдавало предложение текстом. Три
  возвращали жёстко вписанную единицу и слали «найдено 1 подходящих клиентов»
  под категорией «возможность». Автопилот отвечал `dispatched`, не поставив в
  очередь ничего, хотя комментарий в самой функции утверждал обратное.
- Consequence: цикл замыкается и со стороны автоматизаций, а не только со стороны
  сигналов: правило приводит владельца к готовому предложению с числами, которое
  он открывает, правит и запускает обычным путём. Уведомление, которому нечего
  сказать, не отправляется — список уведомлений остаётся читаемым.
