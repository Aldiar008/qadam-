...
```
QADAM_DEMO_TENANTS_ENABLED=false   # true на установке, которая показывает демо
...
`QADAM_DEMO_TENANTS_ENABLED` — **потолок установки**, а не решение о поведении. Он управляет
только тем, предлагается ли демонстрационный вход и открыт ли маршрут `/demo`. Что позволено
конкретному заведению, решает колонка `public.businesses.mode`, и это решение держит база:
констрейнт `(mode='demo' and is_mock) or (mode='production' and not is_mock)` и триггер
«mock rows are forbidden in production businesses». Поэтому одна установка обслуживает и
демонстрационные, и настоящие заведения сразу, а разделение проходит по строкам.
...
| Помечать заведение `mode='demo'`, если в нём есть настоящие данные | База откажет: строки демо-заведения обязаны быть `is_mock` |
...
## Часть E. Демонстрационные заведения

Судьям и инвесторам нужен работающий продукт по ссылке. Раньше под это
поднималось отдельное третье окружение; теперь это не нужно и больше не так.

Демонстрационное заведение — это строка с `mode='demo'`, живущая в той же базе,
что и настоящие. Совместное проживание допустимо ровно потому, что база не даёт
им смешаться:

1. Констрейнт `(mode='demo' and is_mock) or (mode='production' and not is_mock)`
   на `public.businesses`.
2. Триггер, поднимающий `mock rows are forbidden in production businesses` при
   попытке записать синтетическую строку в боевое заведение.
3. RLS, которая и без того не даёт одному арендатору увидеть другого.

Порядок для установки, которая показывает демо:

1. B1–B3 без изменений — миграции те же.
2. Пометить базу как демонстрационную, один раз и вручную:

   ```sql
   insert into private.demo_environment(note) values ('зачем эта база демонстрационная');
   ```

   Без этой строки `supabase/seed/remote_demo_reset.sql` и
   `supabase/seed/remote_demo_seed.sql` откажутся работать. Установка, которая
   не должна содержать демо-данных, метки не получает — и получить их не может.
3. Наложить seed: `node scripts/apply-remote-sql.mjs <ref> supabase/seed/remote_demo_seed.sql`.
4. Развернуть приложение с `QADAM_DEMO_TENANTS_ENABLED=true`.

Seed создаёт два заведения намеренно разного рода: TAMYR Coffee (`mode='demo'`,
заполненное) и Tenant B Test Cafe (`mode='production'`, пустое). Это не
избыточность — на них `npm run test:modes` и доказывает, что одна сборка
обслуживает оба рода заведений по-разному.

**`QADAM_DEMO_TENANTS_ENABLED=true` — не инцидент.** Он не даёт настоящему
заведению ни одной поблажки: скачок во времени, симуляции и demo-вход
привязаны к `mode` заведения, а не к нему. Он решает единственное — предлагать
ли демонстрационный вход вообще.
